...
export function signInWithGoogle(appName = 'Field Notes') {
  const url = buildGoogleUrl(appName);
  if (!url) {
    console.warn('[google-auth] Missing Google OAuth configuration');
    return false;
  }
...
export async function handleGoogleRedirect() {
  ...
  if (error) return console.error('[google-auth] Sign in failed:', error.message);
  try { window.close(); } catch { /* opened as a full tab */ }
}
