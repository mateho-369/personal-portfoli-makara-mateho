...
export default function AuthPage({ mode }: { mode: 'login' | 'signup' }) {
  ...
  return (
    <div className="relative grid min-h-[calc(100vh-76px)] place-items-center overflow-hidden px-5 py-16">
      ...
      <div className="relative w-full max-w-[470px] overflow-hidden rounded-[1.6rem] bg-[#F8F4E9] p-6 shadow-[0_30px_90px_rgba(43,51,40,.14)] sm:p-9">
        <div className="absolute inset-x-0 top-0 h-[3px] bg-gradient-to-r from-[#6E7C52] via-[#D9A441] to-[#5C7A89]" />
        <span className="grid h-12 w-12 place-items-center rounded-full bg-[#D9A441]/14 text-[#A8761F]">{isSignup ? <Leaf size={21} /> : <CircleUserRound size={21} />}</span>
        <p className="eyebrow mt-7">{isSignup ? 'Come in, stay awhile' : 'Good to see you again'}</p>
        <h1 className="mt-3 font-serif text-4xl tracking-tight sm:text-5xl">{isSignup ? 'Say hello' : 'Welcome back'}</h1>
        <p className="mt-3 text-sm leading-relaxed text-[#687064]">{isSignup ? 'Create an account to chat and follow along.' : 'Sign in to continue your quiet conversation.'}</p>

        <form onSubmit={submit} className="mt-8 space-y-5">
          {isSignup && <label className="field-label">Your name<input className="input-field mt-2" autoComplete="name" placeholder="How should we call you?" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label>}
          <label className="field-label">Email address<input type="email" className="input-field mt-2" autoComplete="email" placeholder="you@example.com" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></label>
          <label className="field-label">Password<div className="relative mt-2"><input type={showPassword ? 'text' : 'password'} className="input-field pr-12" autoComplete={isSignup ? 'new-password' : 'current-password'} placeholder="At least 6 characters" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} /><button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#687064]" aria-label="Toggle password visibility">{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></label>
          {!isSignup && <div className="text-right"><button type="button" onClick={resetPassword} className="text-sm text-[#5C7A89] hover:underline">Forgot password?</button></div>}
          {error && <p className="rounded-lg bg-[#D9A441]/12 px-3 py-2.5 text-sm leading-relaxed text-[#7C5922]">{error}</p>}
          {notice && <p className="rounded-lg bg-[#6E7C52]/10 px-3 py-2.5 text-sm leading-relaxed text-[#52623F]">{notice}</p>}
